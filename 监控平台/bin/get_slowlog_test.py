...
def insert_data():
    ...
    local(AWR_DIR+'/pt-query-digest --user='+user+' --password='+passwd+' --review h='+host+',D='+db+',t=myawr_query_review --review-history h='+host+',D='+db+',t=myawr_query_review_history --no-report --limit=100% --filter=" \$event->{add_column} = length(\$event->{arg}) and \$event->{dbid}='+"'"+str(h_id)+"'"+' and \$event->{Full_scan_cnt}='+"'"+"Yes"+"'"+' and \$event->{ipaddr}= \$event->{\'ip\'}'+'" '+'/tmp/'+ip_addr+'_'+my_port+'_slow.log')
    local(rm  +'/tmp/'+ip_addr+'_'+my_port+'_slow.log')
